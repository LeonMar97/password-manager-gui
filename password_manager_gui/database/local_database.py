...
class LocalDatabase(Database):
    ...
    async def add_password(self,usr:User,pas:Password):
        ...
        if await self.validate_user(usr):
            ...
            if encryption.encrypt_data(main_pass=usr.password, website=pas.website_url, new_password=password_json):
                return {"message": "password add successfully"}, status.HTTP_201_CREATED
            else:
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                    detail="something is wrond with the encryption")
        else:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                                 detail=f"password for user {usr.user_name} is incorect")
    ...
    async def get_passwords(self,usr:User):
        ...
        if decrypted_data:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                detail="something is wrong with the encryption")
